        ...
        try:
            ...
            if run_finbert:
                with st.spinner("Ejecutando FinBERT sobre noticias financieras..."):
                    sentiment_features, classified_news = build_finbert_sentiment_features_from_news(
                        news_df=news_df,
                ...
                st.caption(
                    "Esta comparación evalúa si las variables de sentimiento agregan valor "
                    "frente al modelo técnico sin sentimiento. "
                    "La métrica principal académica sigue siendo F1 Score direccional."
                )

                run_finbert_walkforward = st.checkbox(
                    "Ejecutar comparación con FinBERT",
                    value=False,
                    help="Puede tardar porque entrena modelos en ventanas temporales.",
                )
                ...
                if run_finbert_walkforward:
                    ...
                    if "F1 Score" in wf_finbert_results.columns:
                        ...
                        fig_finbert_wf = go.Figure()

                        fig_finbert_wf.add_trace(
                            go.Bar(
                                x=plot_df["Modelo"],
                                y=plot_df["F1 Score"],
                                name="F1 Score",
                        ...
                        fig_finbert_wf.update_layout(
                            template="plotly_dark",
                            title="F1 Score walk-forward: técnico vs técnico + FinBERT",
                            xaxis_title="Modelo",
                            yaxis_title="F1 Score (%)",
                        )
                    ...
                    st.info(
                        "La conclusión académica no debe basarse solo en esta tabla. "
                        "En el siguiente paso se extenderán las pruebas estadísticas para comparar "
                        "modelo técnico sin sentimiento vs modelo técnico + FinBERT."
                    )
